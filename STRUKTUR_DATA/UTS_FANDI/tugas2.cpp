...
int linearSearch(Mahasiswa arr[], int n, int target)
{
    for (int i = 0; i < n; i++)
    {
...
{
    cout << setw(20) << left << "Buku" << setw(10) << left << "Kode Buku" << endl;
    cout << setfill('-') << setw(30) << "-" << endl; // Mencetak garis pemisah
    cout << setfill(' ');                            // Mengembalikan pengisian ke spasi

    for (int i = 0; i < size; i++)
    {
        cout << setw(20) << left << data[i].buku << setw(10) << left << data[i].kodeBuku << endl;
...
int main()
{
    ...
    do
    {
        ...
        case 3:
            ...
            int index = linearSearch(data, MAX, target);
            if (index != -1)
            {

...
void update_barang(int id, string nama_barang, int jumlah_barang, string harga_barang)
{
    MYSQL *conn = connect_db();
    if (conn)
    {
        string query = "UPDATE barang SET nama_barang='" + nama_barang + "', jumlah_barang=" + to_string(jumlah_barang) + ", harga_barang=" + harga_barang + " WHERE id=" + to_string(id);
        if (mysql_query(conn, query.c_str()))
        {
